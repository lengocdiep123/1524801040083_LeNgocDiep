...
{
    public partial class DaoTao_CTDT_CDNChitiet : System.Web.UI.Page
    {
        HtmlHead headTag;
        int numberpage = 12;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                DataTable dttieude = DaoTaoCTDTServiecs.db.DaoTaoCTDT_GetByTop("", "ID=1", "");
                if (dttieude.Rows.Count > 0)
                {
                    lbchudetieude.Text = dttieude.Rows[0]["Chude"].ToString() +" Chương trình chi tiết";
                    headTag.Title = dttieude.Rows[0]["Chude"].ToString() + " - Chương trình chi tiết - Trường Trung Cấp Nghề Củ Chi";
                }


                DataTable dt = DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_GetByTop("", "IDChude=1 and IDLoai=2 and dangtin=1", "ID desc");
                if (dt.Rows.Count > 0)
                {
        ...
        DataTable BindGirdListFile(string IDTin)
        {
            ...
            dt = DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_GetByTop("", "IDTin=" + IDTin, "");
            return dt;
        }
        ...
        protected void lbtlinkdownload_Command(object sender, CommandEventArgs e)
        {
            ...
            {
                string ID = e.CommandArgument.ToString();
                DataTable dt = DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_GetByTop("", "ID=" + ID, "");
