...
{
    public partial class listTindaotao : System.Web.UI.Page
    {
        ...
        {
            grdList.DataSource = DaoTaoTinServiecs.db.DaoTaoTintuc_GetByChude(idchude);
            grdList.DataBind();
        }
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = DaoTaoTinServiecs.db.DaoTaoTintuc_GetByTop("", "ID=" + temp, "");
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    DaoTaoTinServiecs.db.DaoTaoTintuc_Delete(temp);

                    DataTable dtFile = DaoTaoFileServices.db.DaoTaoFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        DaoTaoFileServices.db.DaoTaoFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
        ...
        protected void bt_postnews_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    {
                        DaoTaoTinInfo obj = new DaoTaoTinInfo();
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                DaoTaoTinServiecs.db.DaoTaoTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
        ...
        protected void bt_savenews_Click(object sender, EventArgs e)
        {
            if (bt_savenews.Text == "Thêm tin")
            {
                try
                {
                    ...
                    obj.dangtin = (0).ToString();
                    DaoTaoTinServiecs.db.DaoTaoTintuc_Insert(obj);
                    pnList.Visible = true;
                    pnCreate.Visible = false;
                    DataTable dt = DaoTaoTinServiecs.db.DaoTaoTintuc_GetByTop("1", "", "ID DESC");
                    lbTieudethemfile.Text = dt.Rows[0]["tieude"].ToString();
            ...
            else
            {
                ...
                obj.ngaysua = DateTime.Now;
                obj.dangtin = (0).ToString();
                DaoTaoTinServiecs.db.DaoTaoTintuc_Update(obj);
                pnList.Visible = true;
                pnCreate.Visible = false;
        ...
        protected void bt_savefile_Click(object sender, EventArgs e)
        {
            try
            {
                DaoTaoFileInfo obj = new DaoTaoFileInfo();
                ...
                if (upfile1.HasFile)
                {
                    if (CheckFileTypeDocuments(upfile1.FileName))
                    {
                        ...
                        DaoTaoFileServices.db.DaoTaoFile_Insert(obj);
                        pnlistfile.Visible = true;
                        pnattachfile.Visible = false;
        ...
        {
            grdattachfiles.DataSource = DaoTaoFileServices.db.DaoTaoFile_GetByTop("", "IDTin=" + IDTin, "");
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = DaoTaoFileServices.db.DaoTaoFile_GetByTop("", "ID=" + temp, "");
                    if (dtFile != null)
                    {
                        string pathFile = dtFile.Rows[0]["Duongdan"].ToString();
                        DaoTaoFileServices.db.DaoTaoFile_Delete(temp);
