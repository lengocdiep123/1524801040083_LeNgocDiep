...
{
    public partial class listTincongdoan : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");
            
            if ((int.Parse(Session["Role"].ToString()) == 1) || (int.Parse(Session["Role"].ToString()) == 23))
            {
                if (!IsPostBack)
                {
                    pnList.Visible = true;
        ...
        {
            grdList.DataSource = TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_GetByChude(idchude);
            grdList.DataBind();
        }
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_GetByTop("", "ID=" + temp, "");
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_Delete(temp);

                    DataTable dtFile = TruongDoanTheFileServices.db.TruongDoanTheFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        TruongDoanTheFileServices.db.TruongDoanTheFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
        ...
        protected void bt_postnews_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    {
                        TruongDoanTheTinInfo obj = new TruongDoanTheTinInfo();
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
        ...
        protected void bt_savenews_Click(object sender, EventArgs e)
        {
            if (bt_savenews.Text == "Thêm tin")
            {
                try
                {
                    ...
                    obj.dangtin = (0).ToString();
                    TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_Insert(obj);
                    pnList.Visible = true;
                    pnCreate.Visible = false;
                    DataTable dt = TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_GetByTop("1", "", "ID DESC");
                    //lbTieudethemfile.Text = dt.Rows[0]["tieude"].ToString();
            ...
            else
            {
                ...
                obj.ngaysua = DateTime.Now;
                obj.dangtin = (0).ToString();
                TruongDoanTheTinServiecs.db.TruongDoanTheTintuc_Update(obj);
                pnList.Visible = true;
                pnCreate.Visible = false;
        ...
        protected void bt_savefile_Click(object sender, EventArgs e)
        {
            try
            {
                TruongDoanTheFileInfo obj = new TruongDoanTheFileInfo();
                ...
                if (upfile1.HasFile)
                {
                    if (CheckFileTypeDocuments(upfile1.FileName))
                    {
                        ...
                        TruongDoanTheFileServices.db.TruongDoanTheFile_Insert(obj);
                        pnlistfile.Visible = true;
                        pnattachfile.Visible = false;
        ...
        protected void BindGridFile(string IDTin)
        {
            ...
            grdattachfiles.DataSource = TruongDoanTheFileServices.db.TruongDoanTheFile_GetByTop("", "IDTin=" + IDTin, "");
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = TruongDoanTheFileServices.db.TruongDoanTheFile_GetByTop("", "ID=" + temp, "ID desc");
                    if (dtFile != null)
                    {
                        string pathFile = dtFile.Rows[0]["Duongdan"].ToString();
                        TruongDoanTheFileServices.db.TruongDoanTheFile_Delete(temp);
