...
{
    public partial class listTintttshoso : System.Web.UI.Page
    {
        ...
        {
            grdList.DataSource = TSHoSoTinServiecs.db.TSHoSoTintuc_GetByChude(idchude);
            grdList.DataBind();
        }
        ...
        protected void bt_new_Click(object sender, EventArgs e)
        {
            ...
            ddlChudenew.SelectedValue = ddlChude.SelectedValue;
        }
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = TSHoSoTinServiecs.db.TSHoSoTintuc_GetByTop("", "ID=" + temp, "");
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    TSHoSoTinServiecs.db.TSHoSoTintuc_Delete(temp);

                    DataTable dtFile = TSHoSoFileServices.db.TSHoSoFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        TSHoSoFileServices.db.TSHoSoFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
        ...
        protected void bt_postnews_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    {
                        TSHoSoTinInfo obj = new TSHoSoTinInfo();
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                TSHoSoTinServiecs.db.TSHoSoTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
        ...
        protected void bt_savenews_Click(object sender, EventArgs e)
        {
            if (bt_savenews.Text == "Thêm tin")
            {
                try
                {
                    ...
                    obj.dangtin = (0).ToString();
                    TSHoSoTinServiecs.db.TSHoSoTintuc_Insert(obj);
                    pnList.Visible = true;
                    pnCreate.Visible = false;
                    DataTable dt = TSHoSoTinServiecs.db.TSHoSoTintuc_GetByTop("1", "", "ID DESC");
                    lbTieudethemfile.Text = dt.Rows[0]["tieude"].ToString();
            ...
            else
            {
                ...
                obj.ngaysua = DateTime.Now;
                obj.dangtin = (0).ToString();
                TSHoSoTinServiecs.db.TSHoSoTintuc_Update(obj);
                pnList.Visible = true;
                pnCreate.Visible = false;
        ...
        protected void bt_savefile_Click(object sender, EventArgs e)
        {
            try
            {
                TSHoSoFileInfo obj = new TSHoSoFileInfo();
                ...
                if (upfile1.HasFile)
                {
                    if (CheckFileTypeDocuments(upfile1.FileName))
                    {
                        ...
                        TSHoSoFileServices.db.TSHoSoFile_Insert(obj);
                        pnlistfile.Visible = true;
                        pnattachfile.Visible = false;
        ...
        {
            grdattachfiles.DataSource = TSHoSoFileServices.db.TSHoSoFile_GetByTop("", "IDTin=" + IDTin, "");
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = TSHoSoFileServices.db.TSHoSoFile_GetByTop("", "ID=" + temp, "");
                    if (dtFile != null)
                    {
                        string pathFile = dtFile.Rows[0]["Duongdan"].ToString();
                        TSHoSoFileServices.db.TSHoSoFile_Delete(temp);
