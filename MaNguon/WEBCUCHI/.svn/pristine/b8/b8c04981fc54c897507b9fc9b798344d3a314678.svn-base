...
{
    public partial class listTindaotaoCTDT : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");

            if (int.Parse(Session["Role"].ToString()) == 1 || int.Parse(Session["Role"].ToString()) == 6)
            {
            }
            else
        ...
        //public string loaiCTDT(string obj)
        //{ 
        //    if(obj.Equals("1"))
        //        return "Chương trình khung";
        //    return "Chương trình chi tiết";
        //}
        //Gird view List news
        protected void BindGrid(string idchude)
        {
            grdList.DataSource = DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_GetByChude(idchude);
            grdList.DataBind();
        }
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_GetByTop("", "ID=" + temp, "");
                if (e.CommandName == "Edit")
                {

                    ddlChudenew.SelectedValue = dt.Rows[0]["IDChude"].ToString();
                    //ddlLoaiCT.SelectedValue = dt.Rows[0]["IDLoai"].ToString();
                    txtTieude.Text = dt.Rows[0]["tieude"].ToString();
                    //string[] tach = txtTieude.Text.ToString().Split('-');
                    //txtTieude.Text = tach[2];
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_Delete(temp);

                    DataTable dtFile = DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
        ...
        protected void bt_postnews_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    {
                        DaoTaoCTDTTinInfo obj = new DaoTaoCTDTTinInfo();
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
        ...
        protected void bt_savenews_Click(object sender, EventArgs e)
        {
            if (bt_savenews.Text == "Thêm tin")
            {
                try
                {
                    ...
                    obj.dangtin = (0).ToString();
                    DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_Insert(obj);
                    pnList.Visible = true;
                    pnCreate.Visible = false;
                    DataTable dt = DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_GetByTop("1", "", "ID DESC");
                    lbTieudethemfile.Text = dt.Rows[0]["tieude"].ToString();
            ...
            else
            {
                ...
                obj.ngaysua = DateTime.Now;
                obj.dangtin = (0).ToString();
                DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_Update(obj);
                pnList.Visible = true;
                pnCreate.Visible = false;
        ...
        protected void bt_savefile_Click(object sender, EventArgs e)
        {
            try
            {
                DaoTaoCTDTFileInfo obj = new DaoTaoCTDTFileInfo();
                ...
                if (upfile1.HasFile)
                {
                    if (CheckFileTypeDocuments(upfile1.FileName))
                    {
                        ...
                        DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_Insert(obj);
                        pnlistfile.Visible = true;
                        pnattachfile.Visible = false;
        ...
        {
            grdattachfiles.DataSource = DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_GetByTop("", "IDTin=" + IDTin, "");
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_GetByTop("", "ID=" + temp, "");
                    if (dtFile != null)
                    {
                        string pathFile = dtFile.Rows[0]["Duongdan"].ToString();
                        DaoTaoCTDTFileServices.db.DaoTaoCTDTFile_Delete(temp);
