...
{
    public partial class listTintaichinh : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");
            //if (int.Parse(Session["Role"].ToString()) != 4) Response.Redirect("~/web.Admin/indexadmin.aspx");
            if (int.Parse(Session["Role"].ToString()) == 1 || int.Parse(Session["Role"].ToString()) == 4)
            {
            }
            else
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = TruongPhongBanTinServiecs.db.TruongPhongBanTintuc_GetByTop("", "ID=" + temp, "");
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    TruongPhongBanTinServiecs.db.TruongPhongBanTintuc_Delete(temp);

                    DataTable dtFile = TruongPhongBanFileServices.db.TruongPhongBanFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        TruongPhongBanFileServices.db.TruongPhongBanFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    if (item.ItemType == ListItemType.AlternatingItem | item.ItemType == ListItemType.Item)
                    {
                        ...
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                TruongPhongBanTinServiecs.db.TruongPhongBanTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
