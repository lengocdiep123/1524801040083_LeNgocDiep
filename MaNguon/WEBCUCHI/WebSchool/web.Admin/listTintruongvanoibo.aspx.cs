...
{
    public partial class listTintruongvanoibo : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");
            //if (int.Parse(Session["Role"].ToString()) != 2) Response.Redirect("~/web.Admin/indexadmin.aspx");
            if (int.Parse(Session["Role"].ToString()) == 1 || int.Parse(Session["Role"].ToString()) == 2)
            {
            }
            else
        ...
        {
            grdList.DataSource = TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_GetByChude(idchude);
            grdList.DataBind();
        }
        ...
        protected void grdList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                lbtempID.Text = temp;
                DataTable dt = TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_GetByTop("", "ID=" + temp, "");
                ...
                else if (e.CommandName == "Delete")
                {
                    ...
                    TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_Delete(temp);

                    DataTable dtFile = TruongGioiThieuFileServices.db.TruongGioiThieuFile_GetByTop("", "IDTin=" + temp, "");
                    if (dtFile != null)
                    {
                        ...
                        TruongGioiThieuFileServices.db.TruongGioiThieuFile_DeletebyIDTin(temp);
                    }
                    BindGrid(ddlChude.SelectedValue);
                }
                else
        ...
        protected void bt_postnews_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < grdList.Items.Count; i++)
                {
                    ...
                    {
                        TruongGioiThieuTinInfo obj = new TruongGioiThieuTinInfo();
                        if (((CheckBox)item.FindControl("ChkSelect")).Checked)
                        {
                            ...
                            try
                            {
                                TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_UpdateDangtin(obj);
                            }
                            catch (Exception ex)
                            {
                                WebMsgBox.Show("Lỗi " + ex.Message);
        ...
        protected void bt_savenews_Click(object sender, EventArgs e)
        {
            if (bt_savenews.Text == "Thêm tin")
            {
                try
                {
                    ...
                    obj.dangtin = (0).ToString();
                    TruongGioiThieuTinServiecs.db.TruongGioithieuTintuc_Insert(obj);
                    pnList.Visible = true;
                    pnCreate.Visible = false;
                    DataTable dt = TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_GetByTop("1", "", "ID DESC");
                    lbTieudethemfile.Text = dt.Rows[0]["tieude"].ToString();
            ...
            else
            {
                ...
                obj.ngaysua = DateTime.Now;
                obj.dangtin = (0).ToString();
                TruongGioiThieuTinServiecs.db.TruongGioiThieuTintuc_Update(obj);
                pnList.Visible = true;
                pnCreate.Visible = false;
        ...
        protected void bt_savefile_Click(object sender, EventArgs e)
        {
            try
            {
                TruongGioiThieuFileInfo obj = new TruongGioiThieuFileInfo();
                ...
                if (upfile1.HasFile)
                {
                    if (CheckFileTypeDocuments(upfile1.FileName))
                    {
                        ...
                        TruongGioiThieuFileServices.db.TruongGioithieuFile_Insert(obj);
                        pnlistfile.Visible = true;
                        pnattachfile.Visible = false;
        ...
        {
            grdattachfiles.DataSource = TruongGioiThieuFileServices.db.TruongGioiThieuFile_GetByTop("", "IDTin=" + IDTin, "");
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = TruongGioiThieuFileServices.db.TruongGioiThieuFile_GetByTop("", "ID=" + temp, "");
                    if (dtFile != null)
                    {
                        string pathFile = dtFile.Rows[0]["Duongdan"].ToString();
                        TruongGioiThieuFileServices.db.TruongGioiThieuFile_Delete(temp);
