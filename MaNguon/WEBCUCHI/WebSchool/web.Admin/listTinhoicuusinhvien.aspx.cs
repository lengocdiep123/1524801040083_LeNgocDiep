...
{
    public partial class listTinhoicuusinhvien : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");
            
            if (int.Parse(Session["Role"].ToString()) == 1 || int.Parse(Session["Role"].ToString()) == 22)
            {
            }
            else
        ...
        protected void grdattachfiles_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtFile = TruongDoanTheFileServices.db.TruongDoanTheFile_GetByTop("", "ID=" + temp, "");
                    if (dtFile != null)
                    {
