...
{
    public partial class listTinttboiduongkynangvaquanhedoanhnghiep : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["Role"] == null) Response.Redirect("~/web.Admin/indexadmin.aspx");
            //if (int.Parse(Session["Role"].ToString()) != 20) Response.Redirect("~/web.Admin/indexadmin.aspx");
            if (int.Parse(Session["Role"].ToString()) == 1 || int.Parse(Session["Role"].ToString()) == 20)
            {
            }
            else
