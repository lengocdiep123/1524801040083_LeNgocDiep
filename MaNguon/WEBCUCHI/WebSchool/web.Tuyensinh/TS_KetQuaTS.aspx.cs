...
{
    public partial class TS_KetQuaTS : System.Web.UI.Page
    {
        HtmlHead headTag;
        int numberpage = 12;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                lbchudetieude.Text = "Kết quả tuyển sinh";
                headTag.Title = "Kết quả tuyển sinh - Trường Trung Cấp Nghề Củ Chi";
               
                DataTable dt = TSKetQuaTinServiecs.db.TSKetQuaTintuc_GetByTop("", "dangtin=1", "ID desc");
                if (dt.Rows.Count > 0)
                {
        ...
        DataTable BindGirdListFile(string IDTin)
        {
            ...
            dt = TSKetQuaFileServices.db.TSKetQuaFile_GetByTop("", "IDTin=" + IDTin, "");
            return dt;
        }
        ...
        protected void lbtlinkdownload_Command(object sender, CommandEventArgs e)
        {
            ...
            {
                string ID = e.CommandArgument.ToString();
                DataTable dt = TSKetQuaFileServices.db.TSKetQuaFile_GetByTop("", "ID=" + ID, "");
