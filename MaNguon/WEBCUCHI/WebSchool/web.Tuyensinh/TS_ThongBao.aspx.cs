...
{
    public partial class TS_ThongBao : System.Web.UI.Page
    {
        HtmlHead headTag;
        int numberpage = 12;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                DataTable dttieude = TSServiecs.db.TS_GetByTop("", "ID=4", "");
                ...
                DataTable dt = TSTinServiecs.db.TSTintuc_GetByTop("", "IDChude=4 and dangtin=1", "ID desc");
                if (dt.Rows.Count > 0)
                {
                    cltpage.PageSize = 12;
