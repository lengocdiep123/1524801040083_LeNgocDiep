...
{
    public partial class DaoTao_CTDT_TDNKhung : System.Web.UI.Page
    {
        HtmlHead headTag;
        int numberpage = 12;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    lbchudetieude.Text = dttieude.Rows[0]["Chude"].ToString() + " Chương trình khung";
                    headTag.Title = dttieude.Rows[0]["Chude"].ToString() + " - Chương trình khung - Trường Trung Cấp Nghề Củ Chi";
                }

                DataTable dt = DaoTaoCTDTTinServiecs.db.DaoTaoCTDTTintuc_GetByTop("", "IDChude=2 and IDLoai=1 and dangtin=1", "ID desc");
                if (dt.Rows.Count > 0)
                {
